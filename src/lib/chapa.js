class ChapaCheckout {
  ...
  renderPhoneInput() {
    ...
    phoneInput.addEventListener("input", (e) =>
      this.validatePhoneNumberOnInput(e)
    );
  ...
  createPaymentMethodElement(method) {
    ...
    element.innerHTML = `
              <img src="${paymentMethod.icon}" alt="${
      paymentMethod.name
    }" class="chapa-payment-icon">
              ${
  ...
  validatePhoneNumber(phoneNumber) {
    ...
    if (
      ...
      telebirrRegex.test(phoneNumber) === false
    ) {
    ...
    if (
      ...
      mpesaRegex.test(phoneNumber) === false
    ) {
  ...
  async open(paymentData) {
    try {
      ...
      const initiateResponse = await fetch(`${this.chapaUrl}`, {
        method: "POST",
        headers: {
  ...
  async verifyPayment(refId) {
    try {
      ...
      while (!isVerified) {
        ...
        const verifyResponse = await fetch(`${this.verifyUrl}`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${this.options.publicKey}`,
          },
  ...
  handleSuccessfulPayment(verifyResult, refId) {
    ...
    if (return_url) {
      ...
      const message =
        this.options.customizations.successMessage || "Payment is successful!";
